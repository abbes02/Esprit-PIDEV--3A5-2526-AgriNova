...
function setSceneMood({ weather, timeOfDay }) {
  const isSunny = weather === "Ensoleill̩";
...
// Camera movement on scroll: keep terrain static, only change camera feel
...
walkTimeline
  .to(camera.position, { y: 2.0, x: 0.6, ease: "none" }, 0)
  .to(camera.rotation, { y: 0.18, x: -0.06, ease: "none" }, 0)
  .to(lookAtTarget, { z: -40, x: -0.6, ease: "none" }, 0);
...
function applyWeatherTime() {
  const weather = weatherSelect ? weatherSelect.value.trim() : "Ensoleill̩";
