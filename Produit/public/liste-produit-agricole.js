...
// Agriech 3D field scene
const { gsap, ScrollTrigger } = window;
const canvas = document.getElementById("bg");

const scene = new THREE.Scene();
// Fog for depth and atmospheric perspective
...
const labelBindings = [];
const popup = document.getElementById("produitPopup");
const popupClose = popup ? popup.querySelector(".popup-close") : null;
const popupTitle = document.getElementById("popupTitle");
const popupPrice = document.getElementById("popupPrice");
const popupRef = document.getElementById("popupRef");
const popupCategorie = document.getElementById("popupCategorie");
const popupQuantite = document.getElementById("popupQuantite");
const popupDescription = document.getElementById("popupDescription");
const popupImage = document.getElementById("popupImage");
const popupQtyInput = document.getElementById("popupQtyInput");
const popupQtyField = document.getElementById("popupQtyField");
const popupBuyForm = document.getElementById("popupBuyForm");
const parcelCountX = 3;
const parcelCountZ = Math.max(1, Math.ceil(parcelLabels.length / parcelCountX));
const parcelSpacingZ = 14;
const fieldWidth = 90;
const fieldLength = Math.max(180, parcelCountZ * parcelSpacingZ + 40);
const fieldCenterZ = -((parcelCountZ - 1) * parcelSpacingZ) / 2;

// Ground (farm field with grass texture)
...
ground.position.z = fieldCenterZ;
...
const furrowGeometry = new THREE.BoxGeometry(0.6, 0.2, fieldLength);
...
for (let z = 0; z < parcelCountZ; z += 1) {
  for (let x = 0; x < parcelCountX; x += 1) {
    ...
    parcel.position.set((x - 1) * 6, -1.2, -z * parcelSpacingZ);
...
parcelLabels.forEach((label) => {
  ...
  labelBindings.push({ label, parcel, t });
});
...
function openPopupFromCard(card) {
  ...
  popupTitle.textContent = card.dataset.nom || "Produit";
  popupPrice.textContent = card.dataset.prix || "";
  popupRef.textContent = card.dataset.ref || "—";
  popupCategorie.textContent = card.dataset.categorie || "—";
  popupQuantite.textContent = card.dataset.quantite || "—";
  popupDescription.textContent = card.dataset.description || "";
  if (popupImage) {
    popupImage.src = card.dataset.image || "/logo.png";
    popupImage.alt = card.dataset.nom || "Produit";
  }
  if (popupBuyForm) {
    popupBuyForm.action = card.dataset.addPanierUrl || "#";
  }
  if (popupQtyInput) {
    popupQtyInput.value = "1";
  }
  if (popupQtyField) {
    popupQtyField.value = "1";
  }
...
if (popupQtyInput && popupQtyField) {
  popupQtyInput.addEventListener("input", () => {
    const value = Math.max(1, Number(popupQtyInput.value || 1));
    popupQtyField.value = String(value);
  });
}

// Long furrows to separate parcels
for (let x = -1; x <= 1; x += 1) {
  const furrow = new THREE.Mesh(furrowGeometry, furrowMaterial);
  furrow.position.set(x * 6 + 2.6, -1.25, fieldCenterZ);
...
// Rows of crops (instanced for performance + wind animation)
const cropMaterial = new THREE.MeshStandardMaterial({
  color: 0x6fbe57,
  roughness: 0.75,
});
const cropGeometry = new THREE.ConeGeometry(0.12, 0.6, 6);
const cropRows = Math.max(8, Math.ceil(fieldLength / 8) + 2);
const cropCols = 10;
const cropCount = cropRows * cropCols;
...
for (let row = 0; row < cropRows; row += 1) {
  const z = -row * 8;
  for (let col = 0; col < cropCols; col += 1) {
    const x = (col - 5) * 1.2;
...
// Subtle atmospheric particles (instanced)
...
// Raycaster for hover/click on parcels
...
// Camera moves forward through the field on scroll (walking feel)
const lookAtTarget = { x: 0, y: -0.6, z: -30 };
...
function updateLookAt() {
  ...
  // Make far cards blurrier + more transparent for clarity
  labelBindings.forEach(({ label, parcel }) => {
    const dz = Math.abs(camera.position.z - parcel.position.z);
    const depthT = THREE.MathUtils.smoothstep(dz, 4, 28);
    const blur = THREE.MathUtils.lerp(0, 5, depthT);
    const opacity = THREE.MathUtils.lerp(1, 0.4, depthT);
