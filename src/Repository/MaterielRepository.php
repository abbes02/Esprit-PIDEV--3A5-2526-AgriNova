...
class MaterielRepository extends ServiceEntityRepository
{
    ...
    public function findReportableForUser(int $userId): array
    {
        ...
        return $this->createQueryBuilder('m')
            ...
            ->andWhere('owner.id = :userId OR (renter.id = :userId AND l.dateDebut <= :today AND l.dateFin >= :today AND (l.statut IS NULL OR l.statut = :enCours))')
            ->setParameter('userId', $userId)
...
SELECT
    (
        (SELECT COUNT(*) FROM location WHERE materiel_id = :materielId)
        +
        (SELECT COUNT(*) FROM panne WHERE materiel_id = :materielId)
    ) AS linked_count
SQL;
